...
#[task]
async fn button_pressed(pin: AnyPin) {
    println!("WOW");
    ...
    loop {
        ...
        info!("Button released! Press duration: {:?}", elapsed);
    }
}
...
async fn main(_spawner: Spawner) {
    ...
    for i in 0..4 {
        let random_bytes = rng.random().to_ne_bytes();
        bytes[i * 4..(i + 1) * 4].copy_from_slice(&random_bytes);
    }

    let uuid = Builder::from_random_bytes(bytes)
        .into_uuid()
        .to_string()
        .substring(0, 8)
        .to_string();
    let file_name = uuid + &".WAV";
    ...
    // _spawner
    //     .spawn(button_pressed(peripherals.GPIO10.degrade()))
    //     .unwrap();
    ...
    async {
        let fs = embedded_fatfs::FileSystem::new(inner, FsOptions::new())
            ...
            .unwrap();
        {
            let mut f = fs.root_dir().create_file(file_name.as_str()).await.unwrap();
    ...
    // button.wakeup_enable(true, WakeEvent::LowLevel);
